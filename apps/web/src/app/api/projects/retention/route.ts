...
import { connectToDatabase, Log, Metric, Span } from "@repo/db";
...
export async function GET(request: Request) {
  try {
    ...
    if (!permission.authorized || !permission.project) {
      return NextResponse.json(
        {
          error: {
            code: permission.error?.code || "FORBIDDEN",
            message: permission.error?.message || "Access denied",
          },
        },
        { status: permission.error?.code === "NOT_FOUND" ? 404 : 403 },
      );
    }
      ...
      return NextResponse.json(
        {
          error: {
            code: permission.error?.code || "FORBIDDEN",
            message: permission.error?.message || "Admin access required",
          },
        },
        { status: permission.error?.code === "NOT_FOUND" ? 404 : 403 },
      );
